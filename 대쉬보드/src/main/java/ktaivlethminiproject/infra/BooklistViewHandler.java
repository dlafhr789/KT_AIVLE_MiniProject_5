...
public class BooklistViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenGenerateSucceeded_then_UPDATE_1(
        @Payload GenerateSucceeded generateSucceeded
    ) {
        try {
            if (!generateSucceeded.validate()) return;
            // view 객체 조회
            Optional<Booklist> booklistOptional = booklistRepository.findById(
                generateSucceeded.getBookId()
            );

            if (booklistOptional.isPresent()) {
                Booklist booklist = booklistOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                booklist.setCoverUrl(generateSucceeded.getCoverUrl());
                booklist.setSummary(generateSucceeded.getSummary());
                booklist.setCategory(
                    String.valueOf(generateSucceeded.getCategory())
                );
                booklist.setPoint(generateSucceeded.getPoint());
                booklist.setDownloadUrl(generateSucceeded.getDownloadUrl());
                // view 레파지 토리에 save
                booklistRepository.save(booklist);
            }
        } catch (Exception e) {
